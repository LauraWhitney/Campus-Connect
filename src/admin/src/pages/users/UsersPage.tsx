    ...
    <div className="max-w-6xl mx-auto animate-fade-in">
      ...
      {loading ? <TableSkeleton cols={6} rows={8} /> : results.length === 0 ? (
        <EmptyState icon={Users}
          title={query ? `No results for "${query}"` : 'No users found'}
            ...
            <tbody>
              {results.map(u => (
                <tr key={u.id} className="table-row">
                  ...
                  <td className="td">
                    <span className={ROLE_BADGE[u.role] ?? 'badge-surface'}>{u.role}</span>
                  </td>
                  <td className="td text-surface-400 hidden sm:table-cell max-w-[140px] truncate">
                    {u.faculty ?? '—'}
                  </td>
