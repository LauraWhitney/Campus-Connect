  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="card p-6 relative overflow-hidden">
        <div className="absolute -right-10 -top-10 w-48 h-48 rounded-full bg-primary-50 blur-2xl pointer-events-none" />
      ...
      {loading ? <StatsSkeleton /> : (
        <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3">
          ...
          <StatCard icon={<UserCheck className="w-5 h-5 text-primary-300" />}  label="Active Clubs"   value={stats!.total_clubs}             color="bg-primary-500/15 border border-primary-500/20" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div className="card p-5 lg:col-span-2">
          ...
          {loading ? <div className="h-52 bg-slate-200/30 rounded-xl animate-pulse" /> : (
            <ResponsiveContainer width="100%" height={200}>
              <BarChart data={overviewData} barSize={28}>
                ...
                <Tooltip contentStyle={CUSTOM_TOOLTIP_STYLE} cursor={{ fill: 'rgba(99,102,241,0.06)' }} />
            ...
            <ResponsiveContainer width="100%" height={200}>
              <PieChart>
                <Pie data={feedbackPie} cx="50%" cy="45%" innerRadius={50} outerRadius={75} paddingAngle={4} dataKey="value">
                  {feedbackPie.map((_, i) => <Cell key={i} fill={i === 0 ? '#d4af37' : '#312e81'} />)}
                </Pie>
                ...
                <tr key={u.id} className="table-row">
                  <td className="td font-medium text-white">
                    <div className="flex items-center gap-2">
                      <div className="w-7 h-7 rounded-full bg-slate-200 border border-surface-600/40 flex items-center justify-center shrink-0">
                        <span className="text-primary-400 text-[10px] font-bold">{u.name.charAt(0).toUpperCase()}</span>
                      </div>
                      {u.name}
                    </div>
                  </td>
                  <td className="td text-surface-400">{u.email}</td>
                  <td className="td">
                    <span className={u.role === 'admin' ? 'badge-brand' : u.role === 'lecturer' ? 'badge-blue' : 'badge-surface'}>
                      {u.role}
                    </span>
                  </td>
                  <td className="td text-surface-400 hidden sm:table-cell">{u.faculty ?? '—'}</td>
                  <td className="td text-surface-500 hidden md:table-cell">
